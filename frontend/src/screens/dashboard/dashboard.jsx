...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const handleFetchTemplates = async () => {
      try {
        const response = await axios.post('http://localhost:3000/get-templates', {
          business_id: data.businessAccountId,
  ...
  return (
    <div className='font-poppins max-w-full md:max-w-[80vw] w-full md:mt-4 px-4 md:px-10 flex flex-col gap-y-4 mt-16 py-3'>
      <div className='w-full flex flex-col md:flex-row justify-between'>
        <div className='text-2xl md:text-3xl font-semibold'>Overview</div>
      ...
      <div className='flex flex-wrap justify-start gap-2'>
        {filteredMonthlyCount.map((data, index) => (
          <div key={index} className={`flex flex-col gap-y-3 md:gap-y-5 ${data.color} bg-[#E3F0FF] w-full md:w-[24.3%] px-4 md:px-7 py-2 md:py-3 text-xs rounded-md`}>
            <div>Messages</div>
            <div className='flex justify-between'>
      ...
      <div className='flex flex-col gap-y-4 md:max-h-56 md:overflow-y-auto'>
        <div className='flex md:justify-between justify-center items-center flex-wrap'>
          <div className='opacity-50 text-xs mr-2 sm:mr-0'>Breakdown by employee</div>
          <select
        ...
        <div className='flex flex-wrap justify-start gap-2'>
          {filteredEmployeeCount.map((data, index) => (
            <div key={index} className={`flex flex-col gap-y-3 ${data.employeeId === '' ? 'hidden' : ''} md:gap-y-5 ${data.color} bg-[#E3F0FF] w-full md:w-[24.3%] px-4 md:px-7 py-2 md:py-3 text-xs rounded-md`}>
              <div>Messages</div>
              <div className='flex justify-between'>
                <div className='font-bold text-sm'>{data.amount}</div>
